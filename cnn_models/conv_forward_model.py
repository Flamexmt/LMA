...
class ConvolForwardNet(nn.Module):
    ...
    def __init__(self, width, height, spec_conv_layers, spec_max_pooling, spec_linear, spec_dropout_rates, activation='relu', numBins=8,
                 useBatchNorm=False,
        ...
        for idx in range(len(spec_conv_layers)):
            ...
            act = self.select_act(activation, numFilters, numBins)
        ...
        for idx in range(len(spec_linear)):
            ...
            act = self.select_act(activation, currNumFeatures, numBins)
            self.activations.append(act)
    ...
    def forward(self, input):
        for idx in range(self.total_num_layers):
            if idx < self.num_conv_layers:
                ...
                input = self.linear_layers[idx-self.num_conv_layers](input)
                
            if self.useBatchNorm:
                input = self.batchNormalizationLayers[idx](input)
                
            input = self.activations[idx](input)
...
def train_model(model, train_loader, test_loader, plot_path, initial_learning_rate = 0.001, use_nesterov=True,
                initial_momentum=0.9, weight_decayL2=0.00022, epochs_to_train=100, print_every=200,
                ...
                mix_with_differentiable_quantization=False, activation='relu'):
        ...
        for epoch in range(start_epoch, epochs_to_train+start_epoch):
            ...
            for idx_minibatch, data in enumerate(train_loader, start=1):
                ...
                if (idx_minibatch) % print_every == 0:
                    ...
                    trn_writer.add_scalar('loss', last_loss_saved, epoch+1)
                    ...
                    if pred_accuracy_epochs:
                        str_to_print += ' Last prediction accuracy: {:2f}%'.format(pred_accuracy_epochs[-1]*100) + ' Max Prediction accuracy: {:2f}%'.format(max_accuracy*100)
                    print(str_to_print)
                    print_loss_total = 0
            ...
            tst_writer.add_scalar('acc', curr_pred_accuracy, epoch+1)
            max_accuracy = max(max_accuracy, curr_pred_accuracy)
            MAX_Memory = max(MAX_Memory, torch.cuda.max_memory_allocated())
            tst_writer.add_scalar('max_acc', max_accuracy, epoch+1)
            ...
            print(' === Epoch: {} - prediction accuracy {:2f}% == new learning rate {:2f} == max memory {:2f}MB == '.format(epoch + 1, curr_pred_accuracy*100, new_learning_rate, MAX_Memory/(1024**2)))
